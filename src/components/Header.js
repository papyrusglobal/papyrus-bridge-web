  ...
  <div className="links_container_mobile">
    {withoutEvents ? null : (
      <Link to="/events" className="link" onClick={onMenuToggle}>
        <i className="icon_events" />
        <span className="link_text">Events</span>
      </Link>
    )}
    <Link to="/status" className="link" onClick={onMenuToggle}>
      <i className="icon_status" />
      <span className="link_text">Status</span>
    </Link>
    {withoutEvents ? null : (
      <Link to="/statistics" className="link" onClick={onMenuToggle}>
        <i className="icon_statistics" />
        <span className="link_text">Statistics</span>
      </Link>
    )}
  </div>
);
...
export class Header extends React.Component {
  ...
  render() {
    ...
    const withoutEvents =
      web3Store.metamaskNet.id === web3Store.homeNet.id.toString()
        ? yn(HOME)
        : yn(FOREIGN);
    return (
      <React.Fragment>
        <header className="header">
          ...
          <div className="container">
            ...
            <div
              style={{ flexShrink: 1, cursor: 'pointer' }}
              onClick={this.toggleShowInfo}
            >
              <img src={infoIcon} title="Information" alt="" />
            </div>
        ...
        {showInfo && (
          <div className="network-info">
            <div className="container">
              <h4>
                Papyrus bridge works with two networks - Ethereum and Papyrus
                Network.
              </h4>
              <p>
                To transfer tonkens from Ethereum - go to the settings of your
                metamask and switch the current network to Ethereum. After that
                - you can send tokens from Ethereum.
              </p>
              <p>
                To transfer tokens from Papyrus Network - you need to do the
                reverse operation and switch the metamask to Papyrus Network{' '}
                <span>https://gateway.papyrus.network</span>
              </p>
            </div>
          </div>
        )}
